...
Le Sprint~4 couvre les semaines S7--S8 (60 stories, 144~SP). Il couvre l'exécution terrain des livraisons (preuves numériques, gestion des incidents), la gestion des trois modes de paiement, le suivi GPS en temps réel avec alertes de retard, le traitement des réclamations avec escalade automatique, et le pilotage via des tableaux de bord KPI journaliers. Le Planificateur et le Manager disposent d'interfaces web dédiées pour piloter l'ensemble de l'activité ; le Chauffeur intervient depuis l'application mobile pour la confirmation terrain.

\clearpage

\section{Sprint Backlog}

\textbf{Objectif du sprint :} Le Chauffeur démarre ses livraisons, valide les paiements et peut travailler en mode hors ligne. Le Planificateur suit la flotte en temps réel sur carte GPS et reçoit des alertes automatiques de retard. Le Manager pilote l'activité journalière via des KPI temps réel. Les clients peuvent soumettre et suivre des réclamations, escaladées automatiquement au Manager si non résolues sous 48~heures. Les notifications multicanal informent toutes les parties prenantes.
