...
Le Sprint~5 couvre les semaines S9--S10 (35 stories, 112~SP). Il complète la plateforme avec des fonctionnalités avancées d'Intelligence Artificielle : six modèles de Machine Learning prédictifs déployés via FastAPI, ainsi qu'un assistant conversationnel (Chatbot) Text-to-SQL permettant d'interroger la base de données en langage naturel.
...
\textbf{Objectif du sprint :} Déployer la couche d'intelligence artificielle de Fleet~AI. Le système prédit la demande par zone et les retards, optimise les tournées et évalue les risques. Un Chatbot conversationnel sécurisé permet d'interagir avec les données analytiques en langage naturel.
